...
# Determine the script directory and navigate to the project root
...
if [ $# -eq 0 ]; then
  ...
  echo ""
  echo "Parameters:"
  echo "  [image-name]      Name of the Docker image (default: mcp-server)"
  ...
  echo ""
  echo "Example: $0 my-mcp-server latest 7501 myusername"
  exit 1
fi

# Set parameters with defaults
IMAGE_NAME=${1:-"mcp-server"}
TAG=${2:-"latest"}
PORT=${3:-"7501"}
DOCKER_USERNAME=$4

# Validate the Docker username is provided
...
echo "Building MCP server image: $IMAGE_NAME:$TAG"
echo "Using base image from Docker Hub: $DOCKER_USERNAME/base-mcp-server:latest"

# Build the Docker image using buildx and load it into local registry
# Using buildx with --load ensures the image is available locally
docker buildx build --load \
  --build-arg DOCKER_USERNAME=$DOCKER_USERNAME \
  -t "$IMAGE_NAME:$TAG" \
  -f docker/Dockerfile .

echo "MCP server image built successfully: $IMAGE_NAME:$TAG"

# Tag the image for Docker Hub (if you want to push it later)
...
echo "To run the container: docker run -p $PORT:7501 $IMAGE_NAME:$TAG"
echo "To run with an environment file: docker run -p $PORT:7501 --env-file .env $IMAGE_NAME:$TAG"
