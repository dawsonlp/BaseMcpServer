...
def configure_cline(backup: bool = True) -> None:
    """Configure Cline extension integration in VS Code."""
...
def configure_claude_desktop(backup: bool = True) -> None:
    """Configure Claude Desktop integration."""
    # Load the server registry
    registry = ServerRegistry.load(get_registry_path())

    # Get the Claude Desktop settings path
    settings_path = get_claude_desktop_settings_path()
    settings_dir = settings_path.parent
    ...
    for name, server in registry.servers.items():
        if isinstance(server, LocalServer):
            ...
            # Claude Desktop doesn't support remote servers in the same way
            console.print(f"[yellow]Warning:[/yellow] Remote server '{name}' skipped - Claude Desktop only supports local servers")
            continue
    ...
    # Save the settings
    settings_path.write_text(json.dumps(settings, indent=2))
    console.print(f"Updated Claude Desktop settings at: [bold]{settings_path}[/bold]")
    
    # Print instructions
    console.print("\n[bold green]Claude Desktop integration configured successfully![/bold green]")
    console.print("You may need to restart Claude Desktop for the changes to take effect.")


def configure_editor(target: str, backup: bool = True) -> None:
    """Configure editor/client integration."""
    if target.lower() == "cline":
        configure_cline(backup)
    elif target.lower() == "claude-desktop":
        configure_claude_desktop(backup)
    else:
        raise ValueError(f"Unsupported target: {target}. Supported targets: 'cline', 'claude-desktop'")
