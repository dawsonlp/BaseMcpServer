"""Factory-based entry point for the loadbearing_youtube MCP server."""
...
from tool_config import get_tools_config, server_lifespan
...
PACKAGE_VERSION = version("loadbearing-youtube-mcp-server")
...
def create_server() -> MCPServer:
    ...
    server = MCPServer(
        name=str(config.get("server", "name", default="loadbearing-youtube")),
...
def _print_help() -> None:
    print(
        "Loadbearing YouTube MCP Server\n\n"
        "Usage: loadbearing-youtube-mcp [stdio|streamable-http|sse]\n"
        "       loadbearing-youtube-mcp --transport [stdio|streamable-http|sse]\n\n"
...
def main() -> None:
    ...
    if transport != "stdio":
        ...
        run_options = {
            "host": host,
            "port": int(config.get("server", "port", default=7502)),
        }
