...
from config import JiraInstance, settings
...
def create_jira_client(instance_name: str | None = None) -> JIRA:
    """
...
def register_tools_and_resources(srv: FastMCP):
    ...
    @srv.tool()
    def list_jira_projects(instance_name: str | None = None) -> dict[str, Any]:
        """
    ...
    @srv.tool()
    def get_issue_details(issue_key: str, instance_name: str | None = None) -> dict[str, Any]:
        """
    ...
    def generate_project_workflow_graph(
        ...
        try:
            ...
            instances = settings.get_jira_instances()
            used_instance = instance_name or settings.get_default_instance_name()
            instances[used_instance].url if used_instance in instances else settings.JIRA_URL
